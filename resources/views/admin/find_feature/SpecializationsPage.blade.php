<style>
.zoom {
        padding: 10px;
        transition: transform .2s;
        width: 80px;
        height: 80px;
    }
    .zoom:hover {
        transform: scale(1.5);
    }

    .button {
        background-color: #86e2d5;
    }

    .button:hover {
        background-color:  #86e2d5;
        }
</style>

@extends('admin.layouts.admin')

@section('content')


<div class="container">
    <div class="btn button mb-3 pb-2 " style="border: 1px solid  #86e2d5;">
     <a href="{{ url('admin/specialization/create') }}" style="color:black;" >Add new specialization</a></div>
    {{--  table to list users  --}}
    <div  class="mt-3 mb-3 text-center ">
    <div  class="reponsive-table">
        <table style="margin-Top:20px ;"  class="table table-sm">
            <tr style="background-color: #86e2d5;"  class=" text-center ">
                <td>Number</td>
                <td>Name</td>
                <td></td>
                <td></td>
                <td>Settings</td>
            </tr>
           
           @foreach ($specializations as $specialization)

                <tr class="text-center">
                    <td scope="col" class="text-info">{{$loop->index +1}}</td>
                    <td scope="col" class="text-info">{{$specialization->name}}</td>
                    <td scope="col" class="text-info"></td>
                    <td scope="col" class="text-info"></td>
                    <td>
                        <a style="border: 1px solid #86e2d5;" class="btn button">Edit</a>
                        <a href="{{ route('specialization.delete',['id' => $specialization->id]) }}"  style="border: 1px solid #86e2d5;" class="btn button">Delete</a>

                    </td>
                </tr>
          @endforeach

        </table>
    </div>
    </div>


{{-- list all clinics in a table --}}

@endsection